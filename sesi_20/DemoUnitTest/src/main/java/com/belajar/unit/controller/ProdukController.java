package com.belajar.unit.controller;
...
public class ProdukController {
	...
	@Autowired
	ProdukServiceImpl produkServiceImpl;
	...
	public ResponseEntity<?> allProduk() {
		...
		if(produkServiceImpl.getAllProduk().size() != 0) {
			return new ResponseEntity<List<Produk>>(produkServiceImpl.getAllProduk(), HttpStatus.OK);
		} else {
	...
	public ResponseEntity<?> getProdukById(@PathVariable Long id) {
		Produk produk = produkServiceImpl.getProduk(id);
	...
	public ResponseEntity<?> update(@RequestBody Produk produk, @PathVariable Long id) {
		try {
			Produk produkLama = produkServiceImpl.getProduk(id);
